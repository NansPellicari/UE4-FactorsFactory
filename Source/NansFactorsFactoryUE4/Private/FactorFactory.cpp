...
UNFactorClientAdapter* UNFactorFactory::GetFactorClient(UObject* WorldContextObject)
{
    ...
    if (myensureMsgf(
            bIsImplementedFactorGI, TEXT("The game instance should implements INFactorsFactoryGameInstance to works")))
    {
...
UNFactorAdapterAbstract* UNFactorFactory::AddFactor(
    UObject* WorldContextObject, UNFactorAdapterAbstract* Factor)
{
    UNFactorClientAdapter* Client = GetFactorClient(WorldContextObject);

    if (Client == nullptr) return Factor;

    Client->AddFactor(Factor->InStack, Factor->GetFactor());
